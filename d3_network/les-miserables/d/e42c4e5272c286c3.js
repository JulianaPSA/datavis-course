// https://observablehq.com/@julianapsa/les-miserables@101
  ...
  main.variable(observer("buildvis")).define("buildvis", ["d3","DOM","dataset","forceSimulation","drag"], function(d3,DOM,dataset,forceSimulation,drag)
{
  ...
  let colorNode = d3.scaleLinear().domain(d3.extent(nodes,d => nLinks(d.id))).range(["#778899","#6A5ACD"]);
  
  let circleRadius = d3.scaleSqrt()
                       .range([4,20])
                       .domain(d3.extent(nodes,d => nLinks(d.id)))
  ...
  const node = svg.append("g")
                  ...
                  .attr("class","node")
                  .attr("text","teste")
                  .attr("r",d => circleRadius(nLinks(d.id)))
                  .attr("fill",d => colorNode(nLinks(d.id)))
                  .call(drag(simulation))
  
  node.append("title")
        .text(d => "Grupo : "+ d.group + ". Número de links: " + nLinks(d.id))
