...
<main role="main" class="container">
    ...
    <div class='row'>
        ...
        <div class="col-6">
          <div id='crimesType-chart'>
            <h5> Number of crimes by type </h5>
          </div>

          <div id='crimesDay-chart'>
...
d3.csv("https://gist.githubusercontent.com/emanueles/13dfe2c1d43e11b5207bb4e6125d71bf/raw/41d14f8fbd9d6cd9e8ad074f8417cff1329ab339/chicago_crimes_sept_2019.csv").then(function(data){
  //let parseDate = d3.utcParse("%m/%d/%a")
...
facts.dimension(d => d.longitude)
  ...
  main.variable(observer("typeDim")).define("typeDim", ["facts"], function(facts){return(
facts.dimension(d => d.type)
)});
  main.variable(observer("crimesByType")).define("crimesByType", ["typeDim"], function(typeDim){return(
typeDim.group()
)});
  main.variable(observer("dayDim")).define("dayDim", ["facts","d3"], function(facts,d3){return(
facts.dimension(d => [d.type,d3.timeDay(d.date)])
)});
  main.variable(observer("crimesByDay")).define("crimesByDay", ["dayDim"], function(dayDim){return(
dayDim.group()
)});
...
d3.scaleOrdinal()
  ...
  main.variable(observer("buildvis")).define("buildvis", ["md","container","dc","colorScale","typeDim","crimesByType","d3","dayDim","dataset","crimesByDay"], function(md,container,dc,colorScale,typeDim,crimesByType,d3,dayDim,dataset,crimesByDay)
{
  let view = md`${container()}`         
  let barChart = dc.barChart(view.querySelector("#crimesType-chart"))  
  let seriesChart = dc.seriesChart(view.querySelector("#crimesDay-chart"))
