...
function zoomToFeature(e) {
  ...
  main.variable(observer("info")).define("info", ["L","homicidesByPopulation","map"], function(L,homicidesByPopulation,map)
{
  // control that shows state info on hover
	...
	infoControl.update = function (feat) {
			this._div.innerHTML = '<h5>Número de homicídios</h5>' +  (feat ?
				'<b>' + feat.properties.NOME + '</b><br />' + homicidesByPopulation.get(feat.properties.NOME) + ' homicídios'
...
d3.csv("https://gist.githubusercontent.com/emanueles/bae10fc42b13886b5a00b79737f2b50d/raw/ffb35abe6d9ba0c4ce4866b827f1916748ab65ef/homicidios_2012.csv").then(function(data) {
  ...
  main.variable(observer("homicidesByPopulation")).define("homicidesByPopulation", ["d3"], function(d3){return(
d3.csv("https://gist.githubusercontent.com/emanueles/8ee225792454a78151d0842a10642a29/raw/9984e32effa99da3cea2ec87a7c9b7b231c71d3c/fortaleza_crimes_populacao.csv").then(function(data) {
  let bairrosPop = d3.map()
  data.forEach(function(d) {
    d.crimesByC = (d.Homicidios/d.Populacao) * 100000
    bairrosPop.set(d.Bairro,+d.crimesByC)
  })
  return bairrosPop
})
)});
  main.variable(observer("style")).define("style", ["colorScale","homicidesByPopulation"], function(colorScale,homicidesByPopulation){return(
function style(feature) {
		 return {
					...
					dashArray: '3',
					fillOpacity: 0.6,
					fillColor: colorScale(homicidesByPopulation.get(feature.properties.NOME))
				};
	}
)});
