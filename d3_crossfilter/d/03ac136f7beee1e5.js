// https://observablehq.com/@julianapsa/d3-com-crossfilter-e-dc-js@395
...
dateDim.group().reduceSum(d => d.google)
  ...
  main.variable(observer("fbDim")).define("fbDim", ["facts"], function(facts){return(
facts.dimension(d => d.facebook)
)});
...
dateDim.group().reduceSum(d => d.facebook)
  ...
  main.variable(observer("fbPercFirstDay")).define("fbPercFirstDay", ["dateDim","fbByDayGroup"], function(dateDim,fbByDayGroup){return(
dateDim.group().reduceSum(d => 100*(d.facebook/fbByDayGroup.all(1)[0].value-1))
)});
  main.variable(observer("googlePercFirstDay")).define("googlePercFirstDay", ["dateDim","googleByDayGroup"], function(dateDim,googleByDayGroup){return(
dateDim.group().reduceSum(d => 100*(d.google/googleByDayGroup.all(1)[0].value-1))
)});
  ...
  main.variable(observer("buildcomposite")).define("buildcomposite", ["md","container","dc","d3","dateDim","googleByDayGroup","fbByDayGroup"], function(md,container,dc,d3,dateDim,googleByDayGroup,fbByDayGroup)
{
  ...
  main.variable(observer("acoes")).define("acoes", ["md","container","dc","d3","dateDim","googlePercFirstDay","fbPercFirstDay"], function(md,container,dc,d3,dateDim,googlePercFirstDay,fbPercFirstDay)
{
  ...
  compositeChart.width(800)
              ...
              .compose([
                  dc.lineChart(compositeChart)
                    .group(googlePercFirstDay, 'Google')
                    .ordinalColors(['steelblue']),
                  dc.lineChart(compositeChart)
                    .group(fbPercFirstDay, 'Facebook')
                    .ordinalColors(['darkorange'])])
  dc.renderAll()
  return view      
}
...
d3.json('https://raw.githubusercontent.com/emanueles/datavis-course/master/assets/files/observable/movies.json').then(function(data){
  ...
  main.variable(observer("facts2")).define("facts2", ["crossfilter","datasetMovies"], function(crossfilter,datasetMovies){return(
crossfilter(datasetMovies)
)});
  main.variable(observer("filmDim")).define("filmDim", ["facts2"], function(facts2){return(
facts2.dimension( d => d.film)
)});
...
facts2.dimension( d => d.genre)
  ...
  main.variable(observer("worldwideGrossDim")).define("worldwideGrossDim", ["facts2"], function(facts2){return(
facts2.dimension( d => d.worldwideGross)
)});
  main.variable(observer("budgetDim")).define("budgetDim", ["facts2"], function(facts2){return(
facts2.dimension( d => d.budget)
)});
  main.variable(observer("worldwideGrossByYearGroup")).define("worldwideGrossByYearGroup", ["yearDim"], function(yearDim){return(
yearDim.group().reduceSum(d => d.worldwideGross)
)});
  main.variable(observer("worldwideGrossByGenreGroup")).define("worldwideGrossByGenreGroup", ["genreDim"], function(genreDim){return(
genreDim.group().reduceSum(d => d.worldwideGross)
)});
  main.variable(observer("by_year")).define("by_year", ["md","container","dc","yearDim","d3","worldwideGrossByYearGroup"], function(md,container,dc,yearDim,d3,worldwideGrossByYearGroup)
{
  ...
  main.variable(observer("by_genre")).define("by_genre", ["md","container","dc","genreDim","d3","worldwideGrossByGenreGroup"], function(md,container,dc,genreDim,d3,worldwideGrossByGenreGroup)
{
